...
function KeyAccess({ entry }) {
  ...
  return <Chip component="a" href={offer.url} target="_blank" rel="noopener noreferrer" clickable size="small" color="secondary" variant="outlined" icon={<VpnKeyRoundedIcon />} label={`Chiave a ${price}`} aria-label={`Chiave disponibile per ${entry.rotation.displayTitle || entry.game.title} a ${price}`} />
}
  ...
  return <Card component="article" sx={{ overflow: 'hidden', bgcolor: 'rgba(10,21,41,.78)' }}>
    <CardContent sx={{ display: 'grid', gridTemplateColumns: { xs: '88px minmax(0,1fr)', sm: '132px minmax(0,1fr)' }, gap: { xs: 1.5, sm: 2.5 }, p: { xs: 1.5, sm: 2.5 }, '&:last-child': { pb: { xs: 1.5, sm: 2.5 } } }}>
      ...
      <Stack spacing={1.25} sx={{ minWidth: 0, overflow: 'hidden' }}>
        ...
        <Stack direction="row" flexWrap="wrap" gap={.75}><Chip size="small" icon={<Groups2RoundedIcon />} label={rotation.playerCountLabel || `${rotation.playerCountMin}–${rotation.playerCountMax} giocatori`} /><Chip size="small" variant="outlined" icon={<SportsEsportsRoundedIcon />} label={modeLabel(rotation.hostMode)} /><Access entry={entry} /><KeyAccess entry={entry} /></Stack>
...
export default function CasualFriday() {
  ...
  return <Stack spacing={{ xs: 2.5, md: 3.5 }} sx={{ maxWidth: 980, mx: 'auto' }}>
    ...
    {playlist && <><Stack spacing={.5}><Typography variant="h4">La scaletta de stasera</Typography><Typography color="text.secondary">In ordine de apparizione, salvo ammutinamenti, crash e gente che deve aggiornà Windows.</Typography>{playlist.entries.some((entry) => entry.keyOffer) && <Typography variant="caption" color="text.secondary" sx={{maxWidth: 760, lineHeight: 1.45, fontStyle: 'italic', fontSize: '0.7rem'}}>Prima dell’acquisto di una key verifica attentamente che l’offerta sia attivabile in Italia, che la chiave sia per Steam e che si tratti di una chiave prodotto o di un regalo, non dell'acquisto di un account Steam separato.</Typography>}</Stack><Stack spacing={1.5}>{playlist.entries.map((entry) => <GameCard key={entry.id} entry={entry} />)}</Stack></>}
  </Stack>
}

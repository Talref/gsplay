 ...
 */
export default function CatalogueGameSearch({
  label = 'Search catalogue',
  onSelect,
  excludeId,
  loadGames = catalogueApi.games
}) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      () =>
        loadGames(query, 1, 12)
          .then(
            (response) =>
              active && setOptions(response.games.filter((game) => game.id !== excludeId))
          )
    ...
    <Autocomplete
      ...
      noOptionsText={
        input.trim().length < 3
          ...
          : error || 'No canonical games found.'
      }
      onChange={(_, game) => {
      ...
      renderOption={(props, game) => {
        const aliases = (game.alternativeTitles || [])
          .filter((title) => !executableAlias.test(title))
          .slice(0, 2)
        return (
      ...
      renderInput={(params) => (
        <TextField
          {...params}
          label={label}
          helperText="Type 3+ characters. Use ↑/↓ and Enter to choose a server result."
          InputProps={{
            ...params.InputProps,
            endAdornment: (
              <>

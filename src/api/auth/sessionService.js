...
const RefreshSession = require('../../core/models/RefreshSession');
...
function assertSecrets(config) {
  if (!config.auth.accessSecret || !config.auth.refreshSecret)
    throw new Error(
      'JWT_ACCESS_SECRET and JWT_REFRESH_SECRET are required to enable authentication'
    );
}
...
function createSessionService(config) {
  ...
  async function issue(user, request) {
    ...
    const refreshToken = jwt.sign(
      { sub: user._id.toString(), sid: session._id.toString(), type: 'refresh' },
  ...
  async function rotate(refreshToken, request, User) {
    let payload;
    try {
      payload = jwt.verify(refreshToken, config.auth.refreshSecret);
    } catch {

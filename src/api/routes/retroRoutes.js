...
} = require('../../core/providers/retroAchievementsClient');
const {
  activateChallenge,
  adminOverview,
  cancelChallenge,
  ...
  refreshChallenge,
  updateDescription
} = require('../../core/services/retroService');
...
function createRetroRouter(config, { retroClient } = {}) {
    ...
    try {
      ...
      if (!profile.userId)
        throw new AppError(
          502,
          'retroachievements_unavailable',
          'RetroAchievements returned no account ID'
        );
      ...
      if (linkedElsewhere)
        throw new AppError(
          409,
          'retroachievements_already_linked',
          'That RetroAchievements account is already linked'
        );
    ...
    try {
      ...
      if (!username)
        throw new AppError(
          ...
          'Link a RetroAchievements account first'
        );
  ...
  router.post('/admin/retroachievements/preview', ...admin, async (req, res, next) => {
    try {
      ...
      res.json({
        game: await previewGame(string(req.body.game, 'game', { max: 512 }), getClient())
      });
    } catch (error) {
  ...
  router.post('/admin/retroachievements/challenges', ...admin, async (req, res, next) => {
    try {
      ...
      res
        .status(201)
        .json({ challenge: await activateChallenge(req.user, game, description, getClient()) });
    } catch (error) {
      next(providerFailure(error, 'RetroAchievements game request failed'));
    }
  });
  ...
  router.put(
    '/admin/retroachievements/challenges/:id/description',
    ...admin,
    async (req, res, next) => {
      try {
        object(req.body);
        ...
        res.json({
          challenge: await updateDescription(
            req.user,
            req.params.id,
            positiveInteger(req.body.version, 'version'),
            string(req.body.description, 'description', { min: 0, max: 2000 })
          )
        });
      } catch (error) {
        next(error);
  ...
  router.post(
    '/admin/retroachievements/challenges/:id/cancel',
    ...admin,
    async (req, res, next) => {
      try {
        object(req.body);
        ...
        res.json({
          challenge: await cancelChallenge(
            ...
            req.params.id,
            positiveInteger(req.body.version, 'version'),
            string(req.body.reason, 'reason', { max: 1000 })
          )
        });
      } catch (error) {
        next(error);
  ...
  router.post(
    '/admin/retroachievements/challenges/:id/refresh',
    ...admin,
    async (req, res, next) => {
      try {
        ...
        if (!challenge)
          throw new AppError(
            404,
            'retro_challenge_not_found',
            'Active Retroclub edition not found'
          );

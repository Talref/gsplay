...
async function ensureMetadataJob(
  canonicalGame,
  { userId, reason = 'discovered', maxAttempts } = {}
) {
  if (!canonicalGame || canonicalGame.metadata?.status !== 'pending') return null;
  const canonicalGameId = canonicalGame._id.toString();
  const active = await SyncJob.exists({
    provider: 'igdb',
    kind: 'metadata_enrichment',
    'payload.canonicalGameId': canonicalGameId,
  ...
  const revived = await SyncJob.findOneAndUpdate(
    { idempotencyKey, status: { $in: ['failed', 'completed_with_errors'] } },
    {
      $set: {
        status: 'queued',
        runAfter: new Date(),
        ...
        leaseExpiresAt: null,
        diagnostics: [],
        payload: { canonicalGameId, reason },
        ...(maxAttempts ? { maxAttempts } : {})
      },
      $setOnInsert: { userId, provider: 'igdb', kind: 'metadata_enrichment', idempotencyKey }
    },
    { new: true }
  ).select('+payload');
...
async function retryJob(job, { diagnostics = [], counts = {} } = {}, { delayMs } = {}) {
  ...
  const update = {
    ...
    runAfter: terminal
      ? job.runAfter
      : new Date(now.getTime() + Math.max(retryDelayMs(job.attempts), Number(delayMs) || 0)),
    leaseExpiresAt: null,
    workerId: null
  };
  return SyncJob.findOneAndUpdate(

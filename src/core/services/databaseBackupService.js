...
const { COMMUNITY_TIME_ZONE, zonedParts } = require('../time/communityTime');
...
function backupDateKey(now = new Date()) {
  const { year, month, day } = zonedParts(now, COMMUNITY_TIME_ZONE);
...
async function runScheduledDatabaseBackup(options) {
  ...
  if (!backupConfig.directory) return { status: 'unconfigured' };
  if (zonedParts(now, COMMUNITY_TIME_ZONE).hour < backupConfig.hour) return { status: 'not_due' };

...
function fields(data) {
  ...
  if (
    ...
    throw new ApplicationError('invalid', 'invalid_request', 'Rotation fields are invalid');
  }
...
async function createRotation(actor, data, { itadClient } = {}) {
  ...
  if (!game) throw new ApplicationError('not_found', 'not_found', 'Catalogue game was not found');
...
async function createExternalRotation(actor, data, { igdbClient, itadClient } = {}) {
  ...
  if (data.igdbSlug) {
    ...
    if (!metadata) throw new ApplicationError('not_found', 'not_found', 'IGDB game was not found');
    game =
    ...
    if (!title) throw new ApplicationError('invalid', 'invalid_request', 'title is required');
...
async function updateRotation(actor, id, data, { itadClient } = {}) {
  ...
  if (!rotation)
    throw new ApplicationError('not_found', 'not_found', 'Rotation game was not found');
...
async function setVotingEnabled(actor, id, votingEnabled) {
  ...
  if (!rotation)
    throw new ApplicationError('not_found', 'not_found', 'Active rotation game was not found');

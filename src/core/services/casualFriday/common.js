...
const https = (value, field) => {
  if (!/^https:\/\//.test(value || ''))
    throw new ApplicationError('invalid', 'invalid_request', `${field} must be an HTTPS URL`);
};
const keyOfferUrl = (value) => {
  ...
  try {
    ...
    throw new ApplicationError('invalid', 'invalid_request', 'url must be an HTTPS URL');
  }

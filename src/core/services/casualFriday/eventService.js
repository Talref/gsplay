...
async function manageEventDto(event, now = new Date()) {
  ...
  const users = await User.find({
    _id: { $in: responses.map((response) => response.userId) }
  }).select('usernameDisplay');
  const names = new Map(users.map((user) => [String(user._id), user.usernameDisplay]));
  const rsvps = { yes: [], maybe: [], no: [] };
  const voteCounts = new Map(
    event.candidates.map((candidate) => [String(candidate.rotationGameId), 0])
  );
  ...
  Object.values(rsvps).forEach((items) =>
  ...
  return {
    ...
    votingResults: event.candidates
      .map((candidate) => ({
        ...
        votes: voteCounts.get(String(candidate.rotationGameId))
      }))
...
async function votingCandidates() {
  ...
  if (!rotations.length)
    throw new ApplicationError(
      'invalid',
      'empty_voting_pool',
      'Enable at least one rotation game before starting'
    );
...
async function startEvent(actor, now = new Date()) {
  ...
  if (window.votingClosesAt <= now)
    throw new ApplicationError(
      ...
      'Voting has already closed for this Friday'
    );
  if (await Event.exists({ weekKey: window.weekKey }))
    throw new ApplicationError(
      'conflict',
      'casual_friday_exists',
      'A Casual Friday process already exists for this week'
    );
...
async function requireOpenEvent(eventId, now) {
  ...
  if (!event)
    throw new ApplicationError('not_found', 'not_found', 'Casual Friday event was not found');
  if (!eventIsOpen(event, now))
    throw new ApplicationError(
      'conflict',
      'voting_closed',
      'RSVPs and voting are closed for this event'
    );
  return event;
}
...
async function setVotes(actor, eventId, rotationGameIds, now = new Date()) {
  const event = await requireOpenEvent(eventId, now);
  if (
    rotationGameIds.length > MAX_VOTES ||
    new Set(rotationGameIds).size !== rotationGameIds.length
  )
    throw new ApplicationError(
      'invalid',
      'invalid_votes',
      `Select no more than ${MAX_VOTES} unique games`
    );
  ...
  if (rotationGameIds.some((rotationId) => !candidates.has(rotationId)))
    throw new ApplicationError(
      'invalid',
      'invalid_votes',
      'Votes must belong to the locked candidate list'
    );
  await Response.findOneAndUpdate(
    { eventId: event._id, userId: actor._id },
...
async function createDraft(actor, id, version, now = new Date(), { endVotingEarly = false } = {}) {
  const event = await Event.findOne({ _id: id, status: 'open', version });
  if (!event)
    throw new ApplicationError(
      'conflict',
      'event_not_open',
      'Only the current open event can become a draft'
    );
  ...
  if (event.endsAt <= now)
    throw new ApplicationError(
      'conflict',
      'event_ended',
      'This Casual Friday event has already ended'
    );
  const playlist =
...
async function cancelEvent(actor, id, version, reason, now = new Date()) {
  ...
  if (!event)
    throw new ApplicationError(
      ...
      'This event cannot be cancelled or has changed'
    );
  ...
  await audit(actor, 'event_cancelled', {
    eventId: event._id,
...
async function completeEvent(actor, id, version, now = new Date()) {
  ...
  if (!event)
    throw new ApplicationError(
      ...
      'Only a published event can be completed'
    );

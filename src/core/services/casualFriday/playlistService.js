...
async function updateKeyOffer(
  ...
  if (!entry) {
    ...
    throw new ApplicationError('not_found', 'not_found', 'Playlist entry was not found');
  }
...
async function removeKeyOffer(actor, playlistId, entryId, version, { now = new Date() } = {}) {
  ...
  if (!entry) {
    ...
    throw new ApplicationError('not_found', 'not_found', 'Key offer was not found');
  }
...
async function requireEditablePlaylist(playlistId, version, actor, now = new Date()) {
  ...
  if (!existing) throw new ApplicationError('not_found', 'not_found', 'Playlist was not found');
...
async function updatePlaylistMovie(
  ...
  if (!existing)
    throw new ApplicationError('not_found', 'not_found', 'Movie playlist entry was not found');
...
async function publishPlaylist(actor, id, version, { now = new Date() } = {}) {
  ...
  if (!playlist) throw new ApplicationError('not_found', 'not_found', 'Playlist was not found');

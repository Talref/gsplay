...
const { exactKeys, object } = require('../validation');
...
function requiredString(value, field, max) {
  if (typeof value !== 'string')
    throw new ApplicationError('invalid', 'invalid_server_status', `${field} must be a string`);
...
function normalizeServer(value, index) {
  ...
  if (!STATUSES.has(status))
    throw new ApplicationError(
      'invalid',
      'invalid_server_status',
      `servers[${index}].status is not supported`
    );
...
function normalizeSnapshot(value) {
  ...
  if (
    ...
    throw new ApplicationError(
      'invalid',
      'invalid_server_status',
      'sourceUpdatedAt must be an ISO date string'
    );

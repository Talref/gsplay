...
function gameIdFromInput(value) {
  ...
  try {
    ...
    throw new ApplicationError(
      'invalid',
      'invalid_request',
      'Use a RetroAchievements game URL or numeric game ID'
    );
  }
  if (!/(^|\.)retroachievements\.org$/i.test(url.hostname) || url.username || url.password)
    throw new ApplicationError('invalid', 'invalid_request', 'Use a RetroAchievements game URL');
  const match = url.pathname.match(/^\/game\/(\d+)\/?$/i);
  if (!match)
    throw new ApplicationError(
      'invalid',
      'invalid_request',
      'RetroAchievements game URL is invalid'
    );
...
async function challengeDto(challenge, { includeAchievements = true } = {}) {
  ...
  const averageCompletion = progress.length
    ? Math.round(
        (progress.reduce((sum, entry) => sum + entry.completionPercentage, 0) / progress.length) *
          100
      ) / 100
    : 0;
  const top = ranked[0];
...
async function activateChallenge(actor, input, description, client, now = new Date()) {
  ...
  if (await RetroChallenge.exists({ active: true }))
    throw new ApplicationError(
      'conflict',
      'retro_challenge_active',
      'Cancel the active Retroclub edition before starting another'
    );
  const retroGameId = gameIdFromInput(input);
  const game = await client.getGame(retroGameId);
  if (!game?.title)
  ...
  const last = await RetroChallenge.findOne({ monthKey: window.monthKey })
    .sort({ sequence: -1 })
    .select('sequence');
...
async function updateDescription(actor, id, version, description) {
  ...
  if (!challenge)
    throw new ApplicationError(
      'conflict',
      'retro_challenge_changed',
      'This Retroclub edition changed. Reload it.'
    );
  return challengeDto(challenge);
}
...
async function cancelChallenge(actor, id, version, reason, now = new Date()) {
  ...
  if (!challenge)
    throw new ApplicationError(
      'conflict',
      'retro_challenge_not_cancellable',
      'Only the active Retroclub edition can be cancelled'
    );
  return challengeDto(challenge);
}
...
async function refreshChallenge(challenge, client, now = new Date()) {
  if (!challenge.achievements?.length) {
    ...
    challenge.imageUrl =
      game.imageBoxArt || game.imageTitle || game.imageIcon || challenge.imageUrl;
    challenge.achievements = achievements;
  }
  const users = await User.find({
    'retroAchievements.username': { $exists: true, $ne: '' }
  }).select('usernameDisplay retroAchievements');
  const definitionMap = new Map(
    (challenge.achievements || []).map((item) => [item.achievementId, item])
  );
  let refreshed = 0;
  let failed = 0;
  for (const user of users) {
    try {
      ...
      earned
        .filter(
          (item) =>
            Number(item.gameId) === challenge.retroGameId &&
            definitionMap.has(Number(item.achievementId))
        )
        .forEach((item) => {
          const id = Number(item.achievementId);
          const prior = unique.get(id);
          if (!prior || item.hardcoreMode)
            unique.set(id, {
              achievementId: id,
              earnedAt: new Date(item.date.replace(' ', 'T') + 'Z'),
              hardcore: Boolean(item.hardcoreMode),
              points: definitionMap.get(id).points
            });
        });
      ...
      await RetroProgress.findOneAndUpdate(
        { challengeId: challenge._id, userId: user._id },
        {
          $set: {
            ...
            score,
            completionPercentage: challenge.achievements.length
              ? Math.round((unlocks.length / challenge.achievements.length) * 10_000) / 100
              : 0,
...
async function publicRetroclub(now = new Date()) {
  const [active, lastMonth] = await Promise.all([
    ...
    RetroChallenge.findOne({ monthKey: previousMonthKey(now), status: 'completed' }).sort({
      sequence: -1
    })
  ]);
  return {
...
async function pastEditions(page = 1, pageSize = 6) {
  ...
  const [items, total] = await Promise.all([
    RetroChallenge.find(filter)
      .sort({ monthKey: -1, sequence: -1 })
  ...
  return {
    editions: await Promise.all(
      items.map((item) => challengeDto(item, { includeAchievements: false }))
    ),
    page,
    pageSize,
    total,
...
async function adminOverview() {
  ...
  return {
    accounts: { linked: linkedAccounts, total: totalUsers },
    active: await challengeDto(active),
    editions: await Promise.all(
      recent.map((item) => challengeDto(item, { includeAchievements: false }))
    )
  };
}

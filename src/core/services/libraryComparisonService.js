  ...
  return values.map((value) => {
    if (!mongoose.isObjectIdOrHexString(value))
      throw new ApplicationError(
        'invalid',
        'invalid_request',
        'userIds must contain valid identifiers'
      );
...
function stringList(values, field, maximum = 50) {
  ...
  if (!Array.isArray(values) || values.length > maximum)
    throw new ApplicationError('invalid', 'invalid_request', `${field} must be an array`);
...
function normalizeOptions(data) {
  ...
  if (
    new Set(userIds.map(String)).size !== userIds.length ||
    userIds.length < 2 ||
    userIds.length > 10
  )
    throw new ApplicationError(
      'invalid',
      'invalid_request',
      'Select between two and ten distinct users'
    );
  ...
  multiplayerModes.forEach((mode) => {
    if (!multiplayerModeMap.has(mode))
      throw new ApplicationError(
        'invalid',
        'invalid_request',
        `Unsupported multiplayer mode: ${mode}`
      );
  });
  if (data.multiplayerOnly !== undefined && typeof data.multiplayerOnly !== 'boolean')
    throw new ApplicationError('invalid', 'invalid_request', 'multiplayerOnly must be a boolean');
  return {
    userIds,
    genres,
...
async function compareLibraries(data) {
  ...
  const users = await User.find({ _id: { $in: options.userIds } }, 'usernameDisplay').lean();
  ...
  const [aggregate] = await LibraryItem.aggregate([
    ...
    { $match: { 'game.hiddenAt': null, 'game.archivedAt': null, 'game.mergedIntoId': null } },
    {
      $facet: {
        ...
        gameModes: [{ $unwind: '$game.gameModes' }, { $group: { _id: '$game.gameModes' } }]
      }
    }
  ]);
  ...
  const games = (aggregate?.games || []).map((row) => {
    const owners = selectedUsers.filter((user) =>
      row.ownerIds.some((ownerId) => ownerId.equals(user.id))
    );
    return {
      id: String(row._id),
...
module.exports = {
  MULTIPLAYER_MODES,
  compareLibraries,
  normalizedModes: normalizedMultiplayerModes
};

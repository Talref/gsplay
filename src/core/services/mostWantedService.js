...
async function refreshMostWanted({ steamClient, now = new Date(), log = console }) {
    ...
    try {
      ...
      if (error.retryable && cached?.steamId === user.steamAccount.steamId)
        return {
          ...
          wishlistStatus: cached.wishlistStatus || (cached.appIds.length ? 'accessible' : 'empty'),
          error
        };
      return { user, error };
    }
  ...
  const profileDiagnostics = fetched.map((result) => ({
    ...
    outcome: result.cached ? 'cached' : result.error ? 'unavailable' : result.wishlistStatus,
  ...
  if (users.length && !fresh.length)
    return recordFailure({
      ...
      message:
        'Steam returned no accessible wishlist profiles; the previous snapshot was preserved.'
    });
  ...
  const gameIdByAppId = new Map(candidateMappings.filter(([, gameId]) => gamesById.has(gameId)));

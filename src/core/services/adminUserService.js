...
async function accountCoverage() {
  const [users, snapshot] = await Promise.all([
    User.find({})
      ...
      .sort({ usernameNormalized: 1 })
      .lean(),
    MostWantedSnapshot.findOne({ key: 'current' }).select('lastAttemptAt profileDiagnostics').lean()
  ]);
...
async function updateUserRole({ actor, subjectId, role }) {
  ...
  if (!subject) throw new ApplicationError('not_found', 'not_found', 'User was not found');

...
function createSteamClient({ apiKey, http = axios }) {
  ...
  const providerFailure = (error, operation) => {
    ...
    return new SteamProviderError(`Steam ${operation} request failed with status ${status}`, false);
  };
  return {
    ...
    async probeGameDetails(steamId) {
      ...
      try {
        ...
        const payload = response.data?.response;
        return Boolean(
          payload && (Number.isInteger(payload.game_count) || Array.isArray(payload.games))
        );
      } catch (error) {

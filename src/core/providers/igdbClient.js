...
class IgdbProviderError extends Error {
  constructor(message, retryable = false, status, code, authenticationFailed = false, operation) {
    super(message);
...
function providerError(error, { tokenRequest = false, operation } = {}) {
  ...
  return new IgdbProviderError(
    error.message || 'IGDB request failed',
    retryable,
    status,
    error.code,
...
function mapGame(game) {
  return {
    ...
    alternativeTitles: (game.alternative_names || [])
      .map((item) => item.name)
      .filter(Boolean)
      .slice(0, 50),
    ...
    releaseDate: Number.isInteger(game.first_release_date)
...
function isDesktopGame(game) {
  return game.platforms.some((platform) =>
    ['PC (Microsoft Windows)', 'Linux', 'Mac'].includes(platform)
  );
}
...
function createIgdbClient({ clientId, clientSecret, http = axios.create({ timeout: 10_000 }) }) {
  ...
  async function requestAccessToken() {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        const response = await http.post('https://id.twitch.tv/oauth2/token', null, {
          params: {
            client_id: clientId,
        ...
        tokenExpiresAt =
          Date.now() + Math.max(60, Number(response.data.expires_in || 3600) - 60) * 1000;
        return token;
      } catch (error) {
  ...
  async function accessToken() {
    ...
    if (!clientId || !clientSecret)
      throw new IgdbProviderError(
        'IGDB service credentials are not configured',
        false,
        undefined,
        undefined,
        false,
  ...
  return {
    async searchTitle(title) {
      ...
      try {
        const response = await http.post('https://api.igdb.com/v4/games', query, {
          headers: {
            'Client-ID': clientId,
        ...
        const alternatives = games.filter((game) =>
          game.alternativeTitles.some((alternative) => normalizeTitle(alternative) === normalized)
        );
        ...
        if (desktop.length === 1)
          return { outcome: 'matched', match: desktop[0], candidates: candidates.map(candidateOf) };
        return {
          outcome: candidates.length ? 'ambiguous' : 'not_found',
          candidates: (candidates.length ? candidates : games).map(candidateOf).slice(0, 10)
        };
      } catch (error) {
    ...
    async getGameById(igdbId) {
      if (!Number.isInteger(Number(igdbId)) || Number(igdbId) < 1)
        throw new IgdbProviderError('IGDB game ID must be positive');

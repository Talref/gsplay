...
async function reconcileProviderLibrary({
  userId,
  provider,
  games,
  sourceImportId,
  source = 'api',
  removeAbsent = true,
  enqueueMetadata = false
}) {
  const observedIds = new Set();
  let created = 0;
  let updated = 0;
  for (const game of games) {
    const providerGameId = String(game.providerGameId);
    const providerTitle = String(game.providerTitle).trim();
    ...
    if (alias)
      Object.assign(update, {
        canonicalGameId: alias.canonicalGameId,
        matchStatus: alias.matchType === 'manual' ? 'manually_matched' : 'auto_matched',
        matchConfidence: alias.confidence,
        matchMethod: alias.matchType
      });
    else {
      const candidates = await CanonicalGame.find({
        ...
        .limit(2)
        .lean();
      if (existing?.canonicalGameId && existing.matchStatus !== 'unmatched')
        Object.assign(update, {
          canonicalGameId: existing.canonicalGameId,
        ...
        Object.assign(update, {
          canonicalGameId: candidates[0]._id,
        ...
        const canonical = await CanonicalGame.create({
          canonicalTitle: providerTitle,
          normalizedTitle,
          metadata: { status: 'pending' }
        });
        if (enqueueMetadata)
          await ensureMetadataJob(canonical, { userId, reason: 'library_discovery' });
        Object.assign(update, {
  ...
  const removal = removeAbsent
    ? await LibraryItem.updateMany(
        { userId, provider, removedAt: null, providerGameId: { $nin: [...observedIds] } },

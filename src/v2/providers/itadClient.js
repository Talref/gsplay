...
const EDITION_SUFFIX =
  /\s+(?:complete|deluxe|definitive|ultimate|standard|special|enhanced|anniversary|game of the year|goty)\s+edition$/;

class ItadProviderError extends Error {
  constructor(message, { retryable = false, status, code } = {}) {
    super(message);
    this.name = 'ItadProviderError';
    this.provider = 'itad';
...
function providerError(error) {
  ...
  return new ItadProviderError(error.message || 'ITAD request failed', {
    status,
    code: error.code,
...
function cheapestOffer(item) {
  ...
  const deal = deals
    .filter((candidate) => candidate?.shop?.name && candidate?.url)
    .sort((a, b) => {
...
function createItadClient({
  apiKey,
  country = 'IT',
  http = axios.create({ baseURL: 'https://api.isthereanydeal.com', timeout: 10_000 })
}) {
  function configured() {
    if (!apiKey) throw new ItadProviderError('ITAD API key is not configured');
  }
  return {
    async lookupTitle(title) {
      configured();
      try {
        const { data } = await http.get('/games/search/v1', {
          params: { key: apiKey, title, results: 20 }
        });
        const normalized = normalizeTitle(title);
        const games = (Array.isArray(data) ? data : []).filter(
          (game) => game?.type === 'game' && game?.id && game?.title
        );
        const exact = games.filter((game) => normalizeTitle(game.title) === normalized);
        const candidates = exact.length
          ? exact
          : games.filter(
              (game) => normalizeTitle(game.title).replace(EDITION_SUFFIX, '') === normalized
            );
        if (!candidates.length) return { outcome: 'not_found' };
        if (new Set(candidates.map((game) => String(game.id))).size > 1)
          return { outcome: 'ambiguous' };
        return {
          outcome: 'matched',
          game: { id: String(candidates[0].id), title: String(candidates[0].title) }
        };
      } catch (error) {
        if (error instanceof ItadProviderError) throw error;
    ...
    async bestOffers(gameIds) {
      ...
      try {
        const { data } = await http.post('/games/prices/v3', gameIds, {
          params: { key: apiKey, country, vouchers: true }
        });
        ...
        return new Map(
          gameIds.map((gameId, index) => [
            gameId,
            cheapestOffer(
              byId.get(String(gameId)) || (rows.length === gameIds.length ? rows[index] : undefined)
            )
          ])
        );
      } catch (error) {

...
function accessCookieOptions(config) {
  return {
    ...
    path: '/',
    maxAge: durationMilliseconds(config.auth.accessTtl, 15 * 60 * 1000)
  };
}
function refreshCookieOptions(config) {
  return {
    ...
    sameSite: config.auth.cookieSameSite,
    path: '/api/v2/auth',
    maxAge: durationMilliseconds(config.auth.refreshTtl, 30 * 24 * 60 * 60 * 1000)
  };
}
...
function requireAuth(config) {
  return async (req, res, next) => {
    try {
      const token =
        req.cookies.gsplay_access || req.get('authorization')?.replace(/^Bearer\s+/i, '');

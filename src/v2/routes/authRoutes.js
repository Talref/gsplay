...
function credentials(body, signup = false) {
  object(body);
  exactKeys(body, ['username', 'password']);
  return {
    username: string(body.username, 'username', { min: 3, max: 32 }),
    password: string(body.password, 'password', { min: 8, max: 128 }),
    signup
  };
}
function createAuthRouter(config) {
  ...
  router.post('/signup', mutationLimit, async (req, res, next) => {
    try {
      ...
      if (await User.exists({ usernameNormalized }))
  ...
  router.post('/refresh', mutationLimit, async (req, res, next) => {
    try {
      const result = await sessions.rotate(req.cookies.gsplay_refresh, req, User);
  ...
  router.post('/logout', async (req, res, next) => {
    try {
      await sessions.revoke(req.cookies.gsplay_refresh);
      clearSessionCookies(res, config);
      res.status(204).end();
    } catch (error) {
      next(error);

const request = require('supertest'); const { loadEnvironment } = require('../../src/v2/config/environment'); const { createApp } = require('../../src/v2/app'); const User = require('../../src/v2/models/User'); const Game = require('../../src/v2/models/CanonicalGame'); const Rotation = require('../../src/v2/models/CasualFridayRotationGame'); const Playlist = require('../../src/v2/models/CasualFridayPlaylist'); const Audit = require('../../src/v2/models/CasualFridayAudit'); const { cleanDisplayTitle, completeElapsedPlaylists, nextFridayWindow, refreshRotationOffers } = require('../../src/v2/services/casualFridayService');
const offer = { shop: 'Deal Shop', url: 'https://isthereanydeal.com/game/party/deal', price: 7.49, currency: 'EUR', regularPrice: 24.99, discountPercent: 70, retrievedAt: new Date() }; const config = loadEnvironment({ NODE_ENV: 'test', MONGO_URI: 'mongodb://127.0.0.1:27017/gsplay_test', JWT_ACCESS_SECRET: 'a'.repeat(32), JWT_REFRESH_SECRET: 'b'.repeat(32) }); const itadClient = { lookupTitle: jest.fn().mockResolvedValue({ outcome: 'matched', game: { id: 'itad-party', title: 'Party Game' } }), bestOffers: jest.fn().mockImplementation(async (ids) => new Map(ids.map((id) => [id, offer]))) }; const app = createApp(config, { itadClient }); const password = 'correct-horse-battery-staple';
...
function rotationUpdatePayload(overrides = {}) { const { canonicalGameId, ...payload } = rotationPayload(); return { ...payload, ...overrides }; }

describe('Casual Friday core workflow', () => {
  beforeEach(async () => { jest.clearAllMocks(); await global.testUtils.cleanupDatabase(); });
  test('allows Helpers to manage rotation and rejects members, duplicates, and invalid free overrides', async () => {
    ...
    expect(created.body.rotation).toMatchObject({ displayTitle: 'Party Game', itad: { status: 'verified', gameId: 'itad-party', offer: { price: 7.49 } } }); expect(cleanDisplayTitle('Party Animals - PartyAnimals.exe')).toBe('Party Animals');
  ...
  test('enforces draft/publication lifecycle and optimistic playlist edits', async () => {
    ...
    jest.clearAllMocks();
    const refresh = await refreshRotationOffers({ itadClient, now: new Date('2026-07-31T10:00:00Z') });
    ...
    const updatedRotation = await agent.put(`/api/v2/casual-friday/tools/rotation/${rotations[0].id}`).send(rotationUpdatePayload({ displayTitle: 'Game 0', info: 'Fresh playlist information.', hostMode: 'streamable' })).expect(200);
    expect(updatedRotation.body.rotation.syncedPlaylistEntries).toBe(1);
    const syncedDraft = (await agent.get('/api/v2/casual-friday/tools/playlist').expect(200)).body.playlist;
    expect(syncedDraft.version).toBe(draft.version + 1);
    expect(syncedDraft.entries[0].rotation).toMatchObject({ info: 'Fresh playlist information.', hostMode: 'streamable' });
    ...
    expect(removed.entries[0]).toMatchObject({ owned: false, free: false, rotation: { info: 'Fresh playlist information.', joinInstructions: 'Join the lobby.', hostMode: 'streamable', acquisitionKind: 'owned_store' } });
    expect(await Audit.countDocuments({ playlistId: draft.id, kind: 'playlist_entry_removed' })).toBe(1);
    const betterOffer = { ...offer, price: 2.49, url: 'https://isthereanydeal.com/game/party/better', retrievedAt: new Date() };
    itadClient.bestOffers.mockImplementationOnce(async (ids) => new Map(ids.map((id) => [id, betterOffer])));
    await refreshRotationOffers({ itadClient, now: new Date('2026-07-31T11:00:00Z') });
    const refreshedDraft = (await agent.get('/api/v2/casual-friday/tools/playlist').expect(200)).body.playlist;
    expect(refreshedDraft.entries[0].itad.offer).toMatchObject({ price: 2.49, url: betterOffer.url });
    ...
    const editedPublished = (await agent.delete(`/api/v2/casual-friday/tools/playlist/${draft.id}/entries/${published.entries[0].id}`).send({ version: published.version }).expect(200)).body.playlist;
    expect(editedPublished).toMatchObject({ status: 'published', editable: true });
    await agent.post(`/api/v2/casual-friday/tools/playlist/${draft.id}/cancel`).send({ version: published.version, reason: 'Weather warning' }).expect(409);
    const cancelled = (await agent.post(`/api/v2/casual-friday/tools/playlist/${draft.id}/cancel`).send({ version: editedPublished.version, reason: 'Weather warning' }).expect(200)).body.playlist;
    expect(cancelled).toMatchObject({ status: 'cancelled', editable: false, cancellationReason: 'Weather warning' });
    await agent.delete(`/api/v2/casual-friday/tools/playlist/${draft.id}/entries/${cancelled.entries[0].id}`).send({ version: cancelled.version }).expect(409).expect(({ body }) => expect(body.error.code).toBe('playlist_not_editable'));
    expect(await Audit.countDocuments({ playlistId: draft.id, kind: 'playlist_cancelled' })).toBe(1);
    await agent.post(`/api/v2/casual-friday/tools/playlist/${draft.id}/restore`).send({ version: editedPublished.version }).expect(409);
    const restoredDraft = (await agent.post(`/api/v2/casual-friday/tools/playlist/${draft.id}/restore`).send({ version: cancelled.version }).expect(200)).body.playlist;
    expect(restoredDraft).toMatchObject({ status: 'draft', editable: true });
    expect(restoredDraft.cancellationReason).toBeUndefined();
    const republished = (await agent.post(`/api/v2/casual-friday/tools/playlist/${draft.id}/confirm`).send({ version: restoredDraft.version }).expect(200)).body.playlist;
  ...
  test('builds event boundaries at local Europe/Rome time across DST and keeps Saturday morning on the same event', () => {
    ...
    expect(nextFridayWindow(new Date('2026-01-08T12:00:00Z'))).toMatchObject({
      weekKey: '2026-01-09', startsAt: new Date('2026-01-09T18:00:00Z'), endsAt: new Date('2026-01-10T05:00:00Z')
    });
  ...
  test('retains the last rotation offer when a batched refresh fails', async () => {
    const helper = await user('PriceHelper', 'helper'); const game = await Game.create({ canonicalTitle: 'Price Game', normalizedTitle: 'price game' });
    const rotation = await Rotation.create({ canonicalGameId: game._id, displayTitle: 'Price Game', playerCountMin: 2, playerCountMax: 4, hostMode: 'none', acquisitionKind: 'owned_store', itadStatus: 'verified', itadGameId: 'price-game', itadOffer: offer, addedBy: helper._id, updatedBy: helper._id });
    const failure = { bestOffers: jest.fn().mockRejectedValue(new Error('upstream unavailable')) };
    expect(await refreshRotationOffers({ itadClient: failure })).toMatchObject({ checked: 1, offers: 0, batches: 0 });
    expect(await Rotation.findById(rotation._id)).toMatchObject({ itadOffer: expect.objectContaining({ price: 7.49 }), itadOfferError: 'ITAD price refresh failed' });
  });
});

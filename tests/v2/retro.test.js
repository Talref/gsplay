  ...
  test('links a strict account name and fetches only the caller profile', async () => {
    const retroClient = {
      getProfile: jest.fn().mockResolvedValue({
        user: 'AquaPlayer',
        totalPoints: 1234,
        userPic: 'https://example.test/avatar.png'
      })
    };
    const app = createApp(config, { retroClient });
    await User.create({
      usernameNormalized: 'retro user',
      usernameDisplay: 'Retro User',
      passwordHash: await User.hashPassword(password)
    });
    const agent = request.agent(app);
    await agent.post('/api/v2/auth/login').send({ username: 'Retro User', password }).expect(200);
  ...
  test('lets an admin activate one challenge and gives linked members their own progress DTO', async () => {
    const retroClient = {
      ...
      getGame: jest.fn().mockResolvedValue({
        title: 'Aqua Quest',
        consoleName: 'SNES',
        imageBoxArt: 'https://example.test/aqua.png'
      }),
      getGameProgress: jest
        .fn()
        .mockResolvedValue({ numAwardedToUser: 3, numAchievements: 12, pointsEarned: 25 })
    };
    const app = createApp(config, { retroClient });
    const admin = await User.create({
      usernameNormalized: 'retro admin',
      usernameDisplay: 'Retro Admin',
      passwordHash: await User.hashPassword(password),
      role: 'admin'
    });
    await User.create({
      usernameNormalized: 'retro member',
      usernameDisplay: 'Retro Member',
      passwordHash: await User.hashPassword(password),
      retroAchievements: { username: 'AquaPlayer', linkedAt: new Date() }
    });
    const adminAgent = request.agent(app);
    await adminAgent
      .post('/api/v2/auth/login')
      .send({ username: 'Retro Admin', password })
      .expect(200);
    const created = await adminAgent
      .put('/api/v2/admin/retroachievements/challenge')
      .send({ retroGameId: 123, description: 'June challenge' })
      .expect(200);
    ...
    await memberAgent
      .post('/api/v2/auth/login')
      .send({ username: 'Retro Member', password })
      .expect(200);

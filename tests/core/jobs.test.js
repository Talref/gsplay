const {
  ...
  enqueueJob,
  retryJob
} = require('../../src/core/jobs/jobService');
...
describe('durable job service', () => {
  ...
  test('deduplicates enqueueing and leases work to one worker', async () => {
    const first = await enqueueJob({
      provider: 'steam',
      kind: 'provider_sync',
      idempotencyKey: 'steam:user:1',
      payload: { steamId: '1' }
    });
    const repeated = await enqueueJob({
      provider: 'steam',
      kind: 'provider_sync',
      idempotencyKey: 'steam:user:1',
      payload: { steamId: '1' }
  ...
  test('persists diagnostics and completes a claimed job', async () => {
    ...
    const completed = await completeJob(claimed, {
      diagnostics: [{ code: 'invalid_row', message: 'Row 2 is malformed' }],
      counts: { discovered: 2, failed: 1 }
    });
  ...
  test('reschedules retryable work with exponential backoff before terminal failure', async () => {
    ...
    const retried = await retryJob(first, {
      diagnostics: [{ code: 'temporary_failure', message: 'Try again' }]
    });
    ...
    retried.runAfter = new Date(0);
    await retried.save();
    const second = await claimNextJob('worker-a');
    second.attempts = second.maxAttempts;
    await second.save();
    const terminal = await retryJob(second, {
      diagnostics: [{ code: 'temporary_failure', message: 'Still unavailable' }]
    });
    expect(terminal.status).toBe('failed');
  });

  test('claims a user-visible sync ahead of ready metadata enrichment work', async () => {
    const metadata = await enqueueJob({
      provider: 'igdb',
  ...
  test('defers a provider-paused job without consuming an attempt and can exclude metadata claims', async () => {
    const metadata = await enqueueJob({
      provider: 'igdb',
      kind: 'metadata_enrichment',
      idempotencyKey: 'metadata:paused'
    });
    const claimed = await claimNextJob('worker-a');
    const deferred = await deferJob(claimed, {
      diagnostics: [{ code: 'igdb_request_failed', message: 'Too Many Requests' }]
    });

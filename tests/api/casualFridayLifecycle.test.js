...
const Game = require('../../src/core/models/CanonicalGame');
const Event = require('../../src/core/models/CasualFridayEvent');
const Playlist = require('../../src/core/models/CasualFridayPlaylist');
const PlaylistEntry = require('../../src/core/models/CasualFridayPlaylistEntry');
const Response = require('../../src/core/models/CasualFridayResponse');
const Rotation = require('../../src/core/models/CasualFridayRotationGame');
const Audit = require('../../src/core/models/CasualFridayAudit');
const service = require('../../src/core/services/casualFridayService');
...
async function agentFor(user) {
  ...
  await agent
    ...
    .send({ username: user.usernameDisplay, password })
    .expect(200);
  return agent;
}
...
async function createRotation(actor, index, votingEnabled = true) {
  const game = await Game.create({
    canonicalTitle: `Vote Game ${index}`,
    normalizedTitle: `vote game ${index}`
  });
...
describe('Casual Friday RSVP and voting lifecycle', () => {
  ...
  test('locks the enabled pool and lets helpers and administrators manage voting eligibility', async () => {
    ...
    await helperAgent
      ...
      .send({ enabled: false })
      .expect(200);
    const started = await helperAgent
      .post('/api/v2/casual-friday/tools/event/start')
      .send({})
      .expect(201);
  ...
  test('allows RSVP changes and up to five locked-candidate votes without exposing totals to members', async () => {
    ...
    const rotations = await Promise.all(
      [...Array(6)].map((_, index) => createRotation(helper, index))
    );
    const helperAgent = await agentFor(helper);
    const memberAgent = await agentFor(member);
    const event = (
    ...
    await memberAgent
      ...
      .expect(200);
    await memberAgent
      .put(`/api/v2/casual-friday/events/${event.id}/rsvp`)
      .send({ rsvp: 'maybe' })
      .expect(200);
    await memberAgent
      .put(`/api/v2/casual-friday/events/${event.id}/rsvp`)
  ...
  test('automatically completes a linked event when its published playlist elapses', async () => {
    ...
    await service.createDraft(
      ...
      started.id,
      started.version,
      new Date(started.votingClosesAt)
    );
  ...
  test.each(['open', 'draft', 'published', 'completed'])(
    'can cancel an event from %s state',
    async (state) => {
      ...
      if (state !== 'open') {
        ...
        if (['published', 'completed'].includes(state)) {
          await service.addToPlaylist(helper, rotation._id, {
            now: new Date(event.votingClosesAt)
          });
          const playlist = await Playlist.findById(event.playlistId);
          await service.publishPlaylist(helper, playlist._id, 2, {

...
describe('admin metadata refresh', () => {
  ...
  test('queues and coalesces a confirmed full catalogue metadata refresh', async () => {
    await createUser('Refresh All Member');
    await createUser('Refresh All Admin', 'admin');
    const member = await login('Refresh All Member');
    const admin = await login('Refresh All Admin');
    await member
      .post('/api/v2/admin/enrichment-refresh-all')
      .send({ confirmation: 'REFRESH ALL IGDB METADATA' })
      .expect(403);
    await admin
      .post('/api/v2/admin/enrichment-refresh-all')
    ...
    const first = await admin
      ...
      .expect(202);
    expect(first.body.coalesced).toBe(false);
    expect(await SyncJob.findById(first.body.job._id).select('+payload')).toMatchObject({
      provider: 'igdb',

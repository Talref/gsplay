...
const RetroChallenge = require('../../src/core/models/RetroChallenge');
const RetroProgress = require('../../src/core/models/RetroChallengeProgress');
const { monthWindow, refreshChallenge } = require('../../src/core/services/retroService');
...
describe('Retroclub API', () => {
  ...
  test('verifies and stores the stable provider identity while linking an account', async () => {
    ...
    const response = await agent
      ...
      .expect(200);
    expect(response.body.retroAchievements).toMatchObject({
      username: 'AquaPlayer',
      userId: '4321'
    });
    expect(
      (await User.findOne({ usernameNormalized: 'retro user' })).retroAchievements.userId
    ).toBe('4321');
  ...
  test('prevents one RetroAchievements identity from being linked twice', async () => {
    const retroClient = {
      getProfile: jest.fn().mockResolvedValue({ User: 'AquaPlayer', ID: 4321 })
    };
    const app = createApp(config, { retroClient });
    await user({
      usernameDisplay: 'First User',
      retroAchievements: { username: 'AquaPlayer', userId: '4321', linkedAt: new Date() }
    });
  ...
  test('supports preview, activation, refresh, cancellation, and a replacement in one month', async () => {
    ...
    await user({
      usernameDisplay: 'Retro Member',
      retroAchievements: { username: 'AquaPlayer', userId: '4321', linkedAt: new Date() }
    });
    const admin = await login(app, 'Retro Admin');

    const preview = await admin
      .post('/api/v2/admin/retroachievements/preview')
      .send({ game: 'https://retroachievements.org/game/123' })
      .expect(200);
    expect(preview.body.game).toMatchObject({ retroGameId: 123, achievementCount: 2 });
    const created = await admin
      .post('/api/v2/admin/retroachievements/challenges')
      .send({ game: '123', description: 'Monthly run' })
      .expect(201);
    ...
    await admin
      .post(`/api/v2/admin/retroachievements/challenges/${created.body.challenge.id}/refresh`)
      .send({})
      .expect(200);
    const publicResponse = await admin.get('/api/v2/retroachievements').expect(200);
    expect(publicResponse.body.active.leaderboard[0]).toMatchObject({
      username: 'Retro Member',
      score: 5,
      unlockedCount: 1,
      hardcoreCount: 1,
    ...
    await admin
      .post(`/api/v2/admin/retroachievements/challenges/${active.id}/cancel`)
      .send({ version: active.version, reason: 'Wrong game' })
      .expect(200);
    await admin
      .post('/api/v2/admin/retroachievements/challenges')
      .send({ game: '123', description: '' })
      .expect(201);
  ...
  test('ranks exact scoring ties jointly', async () => {
    ...
    const first = await user({
      usernameDisplay: 'First Player',
      retroAchievements: { username: 'FirstRA', userId: '1', linkedAt: new Date() }
    });
    const second = await user({
      usernameDisplay: 'Second Player',
      retroAchievements: { username: 'SecondRA', userId: '2', linkedAt: new Date() }
    });
    const challenge = await RetroChallenge.create({
      ...
      active: true,
      title: game.title,
      achievements: Object.values(game.achievements).map((item) => ({
        achievementId: item.id,
        badgeId: item.badgeName,
        title: item.title,
    ...
    const client = {
      getAchievementsBetween: jest
        .fn()
        .mockResolvedValue([
          { gameId: 123, achievementId: 10, date: '2026-08-02 10:00:00', hardcoreMode: true }
        ])
    };
    await refreshChallenge(challenge, client, new Date('2026-08-15T12:00:00Z'));
    const progress = await RetroProgress.find({ userId: { $in: [first._id, second._id] } }).sort({
      usernameSnapshot: 1
    });

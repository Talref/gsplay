...
const RefreshSession = require('../../src/core/models/RefreshSession');
...
describe('authentication API', () => {
  ...
  test('signup always creates a member and authenticates the session', async () => {
    ...
    const signup = await agent
      ...
      .send({ username: '  Player One ', password: 'correct-horse-battery-staple', role: 'admin' })
      .expect(400);
    expect(signup.body.error.code).toBe('invalid_request');
    const response = await agent
      .post('/api/v2/auth/signup')
      .send({ username: '  Player One ', password: 'correct-horse-battery-staple' })
      .expect(201);
  ...
  test('rotates the refresh session and revokes it on logout', async () => {
    ...
    await agent
      .post('/api/v2/auth/signup')
      .send({ username: 'Session Player', password: 'correct-horse-battery-staple' })
      .expect(201);
  ...
  test('rejects an invalid login without issuing a session', async () => {
    ...
    const response = await request(app)
      ...
      .send({ username: 'member', password: 'wrong-password' })
      .expect(401);
